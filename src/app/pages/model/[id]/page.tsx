...
export default function ModelPage() {
  ...
  useEffect(() => {
    const fetchModel = async () => {
      try {
        const response = await fetch(`/api/model/${fileId}`);
      ...
      <div className="max-w-4xl mx-auto px-8 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 font-light">
            AI-powered financial modeling from your statements
          </p>
        </div>
          ...
          <div className="px-6 py-6">
            {file.financial_model ? (
              <div className="prose prose-slate max-w-none">
                <ReactMarkdown>{file.financial_model}</ReactMarkdown>
              </div>
            ) : (
              <p className="text-gray-500 italic">No model available for this file.</p>
            )}
          </div>
        </div>
      </div>
